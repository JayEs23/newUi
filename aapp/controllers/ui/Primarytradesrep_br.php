...
class Primarytradesrep_br extends CI_Controller {
	...
	function DisplayReport()
	{
		...
		$qry = "SELECT primary_trades.*,(SELECT user_name FROM investors WHERE (TRIM(investors.email)=TRIM(primary_trades.buy_investor_email)) LIMIT 0,1) AS investor FROM primary_trades WHERE (DATE_FORMAT(tradedate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') AND (TRIM(buy_broker_id)='".$this->db->escape_str($broker_id)."')";
...
//$file = fopen('aaa.txt',"w"); fwrite($file,print_r($qry,true)); fclose($file);
	...
	function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile="reports\primarytradebrreport_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/primarytradebrreport_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="primarytradebrreport_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\primarytradebrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/primarytradebrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="primarytradebrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<tr align="center">										
	...
	<td valign="middle" align="center" width="20%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$bem.'</font></td>
			...
			$pdf->SetKeywords('Naija Art Mart, NSE, Primary Trade, Broker');			
	...
	public function index()
	{
		...
		if (isset($_SESSION['email']))
		{
			...
			$this->load->view('ui/primarytradesrep_br_view',$data);
		}else
		{
