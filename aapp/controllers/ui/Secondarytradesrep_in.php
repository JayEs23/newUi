...
class Secondarytradesrep_in extends CI_Controller {
	...
	function DisplayReport()
	{
		...
		$qry = "SELECT * FROM trades WHERE (DATE_FORMAT(tradedate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') AND ((TRIM(buy_investor_email)='".$this->db->escape_str($email)."') OR (TRIM(sell_investor_email)='".$this->db->escape_str($email)."'))";
		...
		if ($results)
		{
			...
			if (is_array($results))
			{
				foreach($results as $row):
					...
					if ((strtolower(trim($row['buy_investor_email']))) == (strtolower($email)))
					{
						$amt=number_format($row['total_buyer_fee'],2);
					}elseif ((strtolower(trim($row['sell_investor_email']))) == (strtolower($email)))
					{
	...
	function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile="reports\secondarytradereport_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/secondarytradereport_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="secondarytradereport_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\secondarytradereport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/secondarytradereport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="secondarytradereport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<tr align="center">										
	...
	public function index()
	{
		...
		if (isset($_SESSION['email']))
		{
			...
			$this->load->view('ui/secondarytradesrep_in_view',$data);
		}else
		{
