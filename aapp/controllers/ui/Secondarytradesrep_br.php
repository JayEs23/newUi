...
class Secondarytradesrep_br extends CI_Controller {
	...
	{
		$broker_id=''; $symbol=''; $startdate=''; $enddate='';
		...
		$qry = "SELECT * FROM trades WHERE (DATE_FORMAT(tradedate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') AND ((TRIM(sell_broker_id)='".$this->db->escape_str($broker_id)."') OR (TRIM(buy_broker_id)='".$this->db->escape_str($broker_id)."'))";
		...
		if ($results)
		{
			...
			if (is_array($results))
			{
				foreach($results as $row):
					...
					if ((strtolower(trim($row['buy_broker_id']))) == (strtolower($broker_id)))
					{
						$amt=number_format($row['total_buyer_fee'],2);
					}elseif ((strtolower(trim($row['sell_broker_id']))) == (strtolower($broker_id)))
					{
						$amt=number_format($row['total_seller_fee'],2);
					}
	...
	function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile="reports\secondarytradebrreport_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/secondarytradebrreport_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="secondarytradebrreport_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\secondarytradebrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/secondarytradebrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="secondarytradebrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<thead>
   <tr bgcolor="#EEEEEE">
	...
	<td align="right" valign="middle" width="12%"><b><font size="10pt" face="Arial">TOTAL AMOUNT (NGN)</font></b></td>
...
<tr align="center">										
	...
	<td valign="middle" align="center" width="20%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$sem.'</font></td>	
	...
	<td valign="middle" align="center" width="20%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$bem.'</font></td>
			...
			<tfoot>
			  <tr bgcolor="#EEEEEE">
				<th colspan="5" valign="middle" align="right" width="48%"><b><font size="9pt" face="Arial">TOTAL AMOUNT (NGN):&nbsp;</font></b></th>   
	...
	public function index()
	{
		...
		if (isset($_SESSION['email']))
		{
			...
			$this->load->view('ui/secondarytradesrep_br_view',$data);
		}else
		{
