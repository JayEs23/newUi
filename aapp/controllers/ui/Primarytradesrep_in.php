...
class Primarytradesrep_in extends CI_Controller {
	...
	function DisplayReport()
	{
		...
		$qry = "SELECT * FROM primary_trades WHERE (DATE_FORMAT(tradedate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') AND (TRIM(buy_investor_email)='".$this->db->escape_str($email)."')";
...
<tr align="center">										
	...
	<td valign="middle" align="center" width="20%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$bem.'</font></td>
			...
			$pdf->SetKeywords('Naija Art Mart, NSE, Primary Trade, Investor');			
	...
	public function index()
	{
		...
		if (isset($_SESSION['email']))
		{
			...
			$this->load->view('ui/primarytradesrep_in_view',$data);
		}else
		{
