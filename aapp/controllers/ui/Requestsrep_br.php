...
class Requestsrep_br extends CI_Controller {
	...
	{
		$symbol=''; $market_type=''; $trans_type=''; $request_status=''; $startdate=''; 
		$enddate=''; $broker_id='';
		...
		$qry = "SELECT *,(SELECT user_name FROM investors WHERE (TRIM(investors.uid)=TRIM(request_to_buy.investor_id))) AS investor FROM request_to_buy WHERE (DATE_FORMAT(requestdate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') AND (TRIM(broker_id)='".$this->db->escape_str($broker_id)."')";
		...
		if ($results)
		{
			...
			if (is_array($results))
			{
				foreach($results as $row):
					...
					$tp=array($dt,$row['symbol'],number_format($row['tokens'],0),$mxpr,$mnpr,$mtpr,$mtyp,$ttyp,$sta,$row['investor']);
	...
	function CreatePDFReport()
	{
		...
		$market_type=''; $trans_type=''; $request_status=''; $broker_id='';
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile="reports\bsrequestsbrreport_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/bsrequestsbrreport_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="bsrequestsbrreport_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\bsrequestsbrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/bsrequestsbrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="bsrequestsbrreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<thead>
   <tr bgcolor="#EEEEEE">
	...
	<td align="center" valign="middle" width="13%"><b><font size="10pt" face="Arial">INVESTOR</font></b></td>
  </tr>
...
<tr align="center">										
	...
	<td valign="middle" align="center" width="13%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$inv.'</font></td>
</tr>';
	...
	public function index()
	{
		...
		if (isset($_SESSION['email']))
		{
			...
			$this->load->view('ui/requestsrep_br_view',$data);
		}else
		{
