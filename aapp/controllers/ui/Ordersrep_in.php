...
class Ordersrep_in extends CI_Controller {
	...
	{
		$email=''; $symbol=''; $ordertype=''; $transtype=''; $orderstatus=''; $startdate=''; $enddate='';
		...
		$qry = "SELECT * FROM direct_orders WHERE (DATE_FORMAT(orderdate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."') AND (TRIM(investor_id)='".$this->db->escape_str($email)."')";
		...
		if ($results)
		{
			...
			if (is_array($results))
			{
				foreach($results as $row):
					...
					$tp=array($dt,$row['order_id'],$row['symbol'],number_format($row['qty'],0),number_format($row['price'],2),number_format($row['total_amount'],2),$otyp,$row['broker_id'],$ttyp,$sta);
	...
	function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile="reports\ordersinreport_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/ordersinreport_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="ordersinreport_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\ordersinreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/ordersinreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="ordersinreport_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<thead>
   <tr bgcolor="#EEEEEE">
	...
	<td align="center" valign="middle" width="10%"><b><font size="10pt" face="Arial">ORDER ID</font></b></td>
	...
	<td align="center" valign="middle" width="10%"><b><font size="10pt" face="Arial">ORDER TYPE</font></b></td>	
	
	<td align="center" valign="middle" width="10%"><b><font size="10pt" face="Arial">BROKER ID</font></b></td>	
	...
	<td align="center" valign="middle" width="10%"><b><font size="10pt" face="Arial">STATUS</font></b></td>
...
</thead>';//10,10,10,10,10,12,10,10,9,10
			$sn=0;
			...
			{					
				...
				if ($row['ordertype']) $otyp=ucwords(strtolower(trim($row['ordertype'])));
				if ($row['broker_id']) $bid = $row['broker_id'];				
				if ($row['transtype']) $ttyp=ucwords(strtolower(trim($row['transtype'])));
				if ($row['orderstatus']) $sta=ucwords(strtolower(trim($row['orderstatus'])));
...
<tr align="center">										
	...
	<td valign="middle" align="center" width="10%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$oid.'</font></td>	
	...
	<td valign="middle" align="center" width="10%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$otyp.'</font></td>	
	...
	<td valign="middle" align="center" width="10%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$sta.'</font></td>
			...
			$pdf->SetKeywords('Naija Art Mart, NSE, Trade Order, Investor');			
	...
	public function index()
	{
		...
		if (isset($_SESSION['email']))
		{
			...
			$this->load->view('ui/ordersrep_in_view',$data);
		}else
		{
