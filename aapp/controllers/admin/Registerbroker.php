...
class Registerbroker extends CI_Controller {
			...
			}else
			{
				...
				}else
				{
					...
					if ($user['status']==1)
					{
						...
						#Update
						$this->db->trans_start();
						$this->db->where(array('broker_id' => $broker_id));
					...
					if ($query->num_rows() == 0 )
					{
						...
						$altmessage = '
							...
							Naija Art Mart';
						$v= ($to != '') ?$this->getdata_model->SendBlueMail($from,$to,$subject,$Cc,$message,$altmessage,$company) : '';
						// print_r($v); die('User record on Access table');
	...
	public function EditBroker()
	{
		...
		{
			$cm=''; $add=''; $st=''; $em=''; $ph=''; $dt=''; $bid=''; $des=''; $sta='0'; $pwd=''; $badd='';
	...
	public function index()
	{
		...
		if ($_SESSION['email'])
		{
			...
			$this->load->view('admin/registerbroker_view',$data);
		}else
		{
