...
class Tradelogrep_ad extends CI_Controller {
	...
	function LoadUsers()
	{
		$sql="SELECT DISTINCT fullname FROM trade_log ORDER BY fullname";
	...
	function CreateExcelReport()
	{
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile=FCPATH."reports\logtradesrep_ad_report_".str_replace(' ','-',$dstart).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/logtradesrep_ad_report_".str_replace(' ','-',$dstart).".xls";
			}
			
			$filename="logtradesrep_ad_report_".str_replace(' ','-',$dstart).".xls";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile=FCPATH."reports\logtradesrep_ad_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/logtradesrep_ad_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}
			
			$filename="logtradesrep_ad_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
		}
		...
		$qry="SELECT trade_log.*,DATE_FORMAT(logdate,'%d %b %Y @ %H:%i') AS ADate FROM trade_log WHERE (DATE_FORMAT(logdate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."')";
		
		if (trim($username) != '') $qry .= " AND (TRIM(fullname)='".$this->db->escape_str($username)."')";
		
		$qry .= " ORDER BY logdate,`fullname`";
		...
		{		
			...
			{
				echo "There is no trade log record for the selected period.";
			}else
			{
				echo "There is no trade log record.";
			}
		}else
		{				
		############################################# START REPORT HERE #################
			if (count($RegisteredBrokers) > 0)
			{
				...
				$objPHPExcel->getProperties()->setCreator("Naija Art Mart")
											 ...
											 ->setKeywords("Trade Logs")
											 ->setCategory("Trade Logs Report");
				...
				$objPHPExcel->getActiveSheet()->setCellValue('A5', "LOG DATE")
									  ->setCellValue('B5', "OPERATION")
									  ->setCellValue('C5', "ACTIVITY")
									  ->setCellValue('D5', "USER IP ADDRESS");
				...
				$objPHPExcel->getActiveSheet()->getStyle('A5:D5')->getFill()->getStartColor()->setRGB('3D3E11');
				...
				$objPHPExcel->getActiveSheet()->getStyle('A5:D5')->applyFromArray($styleArrayWhite);				
				...
				foreach($RegisteredBrokers as $k => $v):
					...
					if ($v['operation']) $op = $v['operation'].' By '.$v['fullname'];
					...
					$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, $op);
					$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, $act);
					...
					$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':D'.$i)->getFill()->getStartColor()->setRGB('FFFFFF');
					...
					$objPHPExcel->getActiveSheet()->getStyle('A'.$i.':D'.$i)->applyFromArray($styleArrayBlack);	
				endforeach;
				...
				$objPHPExcel->getActiveSheet()->getStyle('A5:D'.$i)->applyFromArray($styleArray);
				...
				$objPHPExcel->setActiveSheetIndex(0);
				$objPHPExcel->getActiveSheet()->setTitle('TRADE LOG REPORT');
			...
			{
				echo 'There is no trade log record for the selected date.';
			}			
		}//End Build Report	
	}
	...
	function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\logtradesrep_ad_report_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/logtradesrep_ad_report_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="logtradesrep_ad_report_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\logtradesrep_ad_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/logtradesrep_ad_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="logtradesrep_ad_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<thead>
   <tr bgcolor="#EEEEEE">
	...
	<td align="center" valign="middle" width="25%"><b><font size="10pt" face="Arial">OPERATION</font></b></td>
	<td align="center" valign="middle" width="49%"><b><font size="10pt" face="Arial">ACTIVITY</font></b></td>
	<td align="center" valign="middle" width="10%"><b><font size="10pt" face="Arial">IP ADDRESS</font></b></td>	
  </tr>
</thead>';//16,25,49,10
			...
			{					
				...
				if ($row['operation']) $op = $row['operation'].' By '.$row['fullname'];
...
<tr align="center">										
	...
	<td valign="middle" align="center" width="25%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$op.'</font></td>	
	<td valign="middle" align="center" width="49%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$act.'</font></td>	
	<td valign="middle" align="center" width="10%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$ip.'</font></td>		
</tr>';
			}
		...
		if ($results)
		{
			...
			if (is_array($results))
			{
				foreach($results as $row):
					$tp=array($row['ADate'],$row['operation'].' By '.$row['fullname'],str_replace("\'","'",html_entity_decode($row['activity'])),$row['remote_ip']);
	...
	public function index()
	{
		...
		if ($_SESSION['email'])
		{
			...
			$this->load->view('admin/tradelogrep_ad_view',$data);
		}else
		{
