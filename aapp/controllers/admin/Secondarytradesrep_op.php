...
class Secondarytradesrep_op extends CI_Controller {
	...
	{
		$buyer=''; $buyerbroker=''; $seller=''; $sellerbroker=''; $symbol='';
		...
		if ($this->input->post('sellerbroker')) $sellerbroker = trim($this->input->post('sellerbroker'));
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile=FCPATH."reports\secondarytrade_op_report_".str_replace(' ','-',$dstart).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/secondarytrade_op_report_".str_replace(' ','-',$dstart).".xls";
			}
			
			$filename="secondarytrade_op_report_".str_replace(' ','-',$dstart).".xls";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile=FCPATH."reports\secondarytrade_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/secondarytrade_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}
			
			$filename="secondarytrade_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
		}
		
		$xls_file=$filename;
		
		$SecondaryTrades=array();
				
		$qry = "SELECT * FROM trades WHERE (DATE_FORMAT(tradedate,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."')";
		...
		$SecondaryTrades = $query->result_array();
						
		if ($query->num_rows() == 0 )
		{		
			if ($startdate and $enddate)
			{
				echo "There is no secondary trade record for the selected period.";
			}else
			{
				echo "There is no secondary trade record.";
			}
		}else
		{				
		############################################# START REPORT HERE #################
			if (count($SecondaryTrades) > 0)
			{
				...
				$objPHPExcel->getProperties()->setCreator("Naija Art Mart")
											 ...
											 ->setKeywords("Secondary Trade Report,Trades")
											 ->setCategory("Secondary Trade Report");
				...
				foreach($SecondaryTrades as $k => $v):
					...
					if ($v['total_buyer_fee'])
					{			
						$amt=number_format($v['total_buyer_fee'],2);
						$tamt += $v['total_buyer_fee'];				
					}
					
					if ($v['sell_investor_email']) $sem = $v['sell_investor_email'];				
				...
				//Merge Agenda Column
				$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,"TOTAL AMOUNT: ")
											  ->mergeCells('A'.$i.':E'.$i);
			...
			{
				echo 'There is no secondary trade record for the selected date.';
			}			
		}//End Build Report	
	}
	...
	public function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\secondarytrade_op_report_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/secondarytrade_op_report_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="secondarytrade_op_report_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\secondarytrade_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/secondarytrade_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="secondarytrade_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<thead>
   <tr bgcolor="#EEEEEE">
	...
	<td align="right" valign="middle" width="9%"><b><font size="10pt" face="Arial">PRICE (NGN)</font></b></td>
	...
	<td align="center" valign="middle" width="21%"><b><font size="10pt" face="Arial">BUYER EMAIL</font></b></td>
  </tr>
</thead>';
			...
			{					
				...
				$amt=number_format($row['total_buyer_fee'],2);
				$tamt += $row['total_buyer_fee'];				
								
				if ($row['sell_investor_email']) $sem = $row['sell_investor_email'];				
...
<tr align="center">										
	...
	<td valign="middle" align="right" width="9%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$pr.'</font></td>
	<td valign="middle" align="right" width="12%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$amt.'</font></td>	
		
	<td valign="middle" align="center" width="21%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$sem.'</font></td>	
		
	<td valign="middle" align="center" width="21%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$bem.'</font></td>
</tr>';
			}
			...
			<tfoot>
			  <tr bgcolor="#EEEEEE">
				<th colspan="5" valign="middle" align="right" width="46%"><b><font size="9pt" face="Arial">TOTAL AMOUNT (NGN):&nbsp;</font></b></th>   
			...
			$pdf->SetKeywords('Naija Art Mart, NSE, Secondary Trade, Trades');
			...
			$pdf->SetHeaderData($logo, 0, 2, 'Secondary Trade Report');
	...
	{
		$buyer=''; $buyerbroker=''; $seller=''; $sellerbroker=''; $symbol=''; $startdate=''; $enddate='';
		...
		if ($results)
		{
			...
			if (is_array($results))
			{
				foreach($results as $row):
					...
					$tp=array($dt,$row['trade_id'],$row['symbol'],number_format($row['num_tokens'],0),number_format($row['trade_price'],2),$amt,$row['sell_investor_email'],$row['buy_investor_email']);
	...
	public function index()
	{
		...
		if ($_SESSION['email'])
		{
			...
			$this->load->view('admin/secondarytradesrep_op_view',$data);
		}else
		{
