...
class Listingsrep_gal extends CI_Controller {
	...
	{
		$status=''; $startdate=''; $enddate=''; $period=''; $ReportTitle='';
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile=FCPATH."reports\listing_gal_report_".str_replace(' ','-',$dstart).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/listing_gal_report_".str_replace(' ','-',$dstart).".xls";
			}
			
			$filename="listing_gal_report_".str_replace(' ','-',$dstart).".xls";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile=FCPATH."reports\listing_gal_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/listing_gal_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}
			
			$filename="listing_gal_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
		}
		...
		{		
			...
			{
				echo "There is no listing request record for the selected period.";
			}else
			{
				echo "There is no listing request record.";
			}
		}else
		{				
		############################################# START REPORT HERE #################
			if (count($Listings) > 0)
			{
				...
				$objPHPExcel->getProperties()->setCreator("Naija Art Mart")
											 ...
											 ->setKeywords("listing Requests Report,listing")
											 ->setCategory("listing Requests Report");
				...
				$objPHPExcel->getActiveSheet()->setCellValue('A5', "ARTIST")
									  ...
									  ->setCellValue('F5', "QTY TO SALE")
									  ->setCellValue('G5', "PRICE")
									  ->setCellValue('H5', "LISTING STARTS")
									  ->setCellValue('I5', "LISTING ENDS")
				...
				foreach($Listings as $k => $v):
					...
					$sn++; $art=''; $tit=''; $sym=''; $val=''; $tok=''; $sqty=''; $pr=''; $sd='';
					$ed=''; $sta='';
					...
					$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $sd);
					$objPHPExcel->getActiveSheet()->setCellValue('I'.$i, $ed);
				...
				$objPHPExcel->setActiveSheetIndex(0);
				$objPHPExcel->getActiveSheet()->setTitle('LISTING REQUESTS REPORT');
			...
			{
				echo 'There is no listing request record for the selected date.';
			}			
		}//End Build Report	
	}
	...
	public function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\listing_gal_report_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/listing_gal_report_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="listing_gal_report_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\listing_gal_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/listing_gal_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="listing_gal_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<tr align="center">										
	...
	<td valign="middle" align="center" width="10%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$sta.'</font></td>
			...
			$pdf->SetKeywords('Naija Art Mart, NSE, Listing Requests, Gallery');			
	...
	public function index()
	{
		...
		if ($_SESSION['email'])
		{
			...
			$this->load->view('admin/listingsrep_gal_view',$data);
		}else
		{
