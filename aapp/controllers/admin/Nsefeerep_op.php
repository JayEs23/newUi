...
class Nsefeerep_op extends CI_Controller {
	...
	function CreateExcelReport()
	{
		...
		if ($startdate==$enddate)
		{
			...
			{
				$outputfile=FCPATH."reports\stockexcchange_op_report_".str_replace(' ','-',$dstart).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/stockexcchange_op_report_".str_replace(' ','-',$dstart).".xls";
			}
			
			$filename="stockexcchange_op_report_".str_replace(' ','-',$dstart).".xls";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile=FCPATH."reports\stockexcchange_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile=FCPATH."reports/stockexcchange_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
			}
			
			$filename="stockexcchange_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".xls";
		}
		...
		$qry = "SELECT * FROM trades_payments WHERE (INSTR(recipient,'NSE') > 0) AND (DATE_FORMAT(trade_date,'%Y-%m-%d') BETWEEN '".$startdate."' AND '".$enddate."')";
		
		$qry .= " ORDER BY trade_date";

		$query = $this->db->query(stripslashes($qry));		
		$NSECommission = $query->result_array();
						
		if ($query->num_rows() == 0 )
		{		
			if ($startdate and $enddate)
			{
				echo "There is no NSE commission record for the selected period.";
			}else
			{
				echo "There is no NSE commission record.";
			}
		}else
		{				
		############################################# START REPORT HERE #################
			if (count($NSECommission) > 0)
			{
				...
				$objPHPExcel->getProperties()->setCreator("Naija Art Mart")
											 ...
											 ->setKeywords("NSE Commission Report,Trades")
											 ->setCategory("NSE Commission Report");
				...
				$objPHPExcel->getActiveSheet()->setCellValue('A5', "TRADE DATE")
									  ...
									  ->setCellValue('G5', "NSE COMMISSION");
				...
				//Merge Agenda Column
				$objPHPExcel->getActiveSheet()->setCellValue('A'.$i,"TOTAL NSE COMMISSION: ")
											  ->mergeCells('A'.$i.':F'.$i);
				...
				$objPHPExcel->setActiveSheetIndex(0);
				$objPHPExcel->getActiveSheet()->setTitle('NSE COMMISSION REPORT');
			...
			{
				echo 'There is no NSE commission record for the selected date.';
			}			
		}//End Build Report	
	}
	...
	function CreatePDFReport()
	{
		...
		if ($startdate==$enddate)
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\stockexcchange_op_report_".str_replace(' ','-',$dstart).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/stockexcchange_op_report_".str_replace(' ','-',$dstart).".pdf";
			}
			
			$filename="stockexcchange_op_report_".str_replace(' ','-',$dstart).".pdf";
		}else
		{
			if (strtolower(trim($_SERVER['HTTP_HOST']))=='localhost')
			{
				$outputfile="reports\stockexcchange_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}elseif (strtolower(trim($_SERVER['HTTP_HOST']))=='www.naijaartmart.com')
			{
				$outputfile="reports/stockexcchange_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
			}
			
			$filename="stockexcchange_op_report_from_".str_replace(' ','-',$dstart).'_to_'.str_replace(' ','-',$dend).".pdf";
		}
...
<thead>
   <tr bgcolor="#EEEEEE">
	...
	<td align="right" valign="middle" width="20%"><b><font size="10pt" face="Arial">NSE COMMISSION (NGN)</font></b></td>
  </tr>
...
<tr align="center">										
	...
	<td valign="middle" align="right" width="20%"><font size="9pt" face="Georgia, Times New Roman, Times, serif">'.$fee.'</font></td>
			...
			<tfoot>
			  <tr bgcolor="#EEEEEE">
				<th colspan="6" valign="middle" align="right" width="80%"><b><font size="9pt" face="Arial">TOTAL NSE COMMISSION (NGN):&nbsp;</font></b></th>   
...
//$file = fopen('aaa.txt',"w"); fwrite($file,print_r($qry,true)); fclose($file);		
	...
	public function index()
	{
		...
		if ($_SESSION['email'])
		{
			...
			$this->load->view('admin/nsefeerep_op_view',$data);
		}else
		{
