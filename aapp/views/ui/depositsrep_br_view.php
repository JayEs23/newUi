		...
		$(document).ready(function(e) {
			...
			$('#btnPDF').click(function(e) 
			{
				try
				{
					...
					if (sdt==edt)
					{
						reportfile="deposit_br_report_"+sd+".pdf";
					}else
					{
						reportfile="deposit_br_report_from_"+sd+'_to_'+ed+".pdf";
					}
					...
					var mydata={email:Email,channel:ch,trans_status:sta,startdate:sdt,enddate:edt,ReportTitle:msg};	
																
					$.ajax({
						url: "<?php echo site_url('ui/Depositsrep_br/CreatePDFReport'); ?>",
						data: mydata,
						type: 'POST',
						dataType: 'text',
			...
			function DisplayReport(ch,sta,sdt,edt)
			{
				try
				{
					timedAlert('Retrieving Wallet Deposits Records',2000,'','info');
					...
					var mydata={email:Email,channel:ch,trans_status:sta,startdate:sdt,enddate:edt};	
					
					$('#recorddisplay > tbody').html('');
					
					$.ajax({
						url: "<?php echo site_url('ui/Depositsrep_br/DisplayReport');?>",
						data: mydata,
						type: 'POST',
						dataType: 'json',
