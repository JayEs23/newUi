		...
		$(document).ready(function(e) {
			...
			$('#btnPDF').click(function(e) 
			{
				try
				{
					...
					if (sdt==edt)
					{
						reportfile="ordersinreport_"+sd+".pdf";
					}else
					{
						reportfile="ordersinreport_from_"+sd+'_to_'+ed+".pdf";
					}
					...
					var mydata={email:Email,symbol:sym,ordertype:otyp,transtype:ttyp,orderstatus:sta,startdate:sdt,enddate:edt,ReportTitle:msg};
																
					$.ajax({
						url: "<?php echo site_url('ui/Ordersrep_in/CreatePDFReport'); ?>",
						data: mydata,
						type: 'POST',
						dataType: 'text',
			...
			function DisplayReport(sym,otyp,ttyp,sta,sdt,edt)
			{
				try
				{
					...
					var mydata={email:Email,symbol:sym,ordertype:otyp,transtype:ttyp,orderstatus:sta,startdate:sdt,enddate:edt};	
					
					$('#recorddisplay > tbody').html('');
					
					$.ajax({
						url: "<?php echo site_url('ui/Ordersrep_in/DisplayReport');?>",
						data: mydata,
						type: 'POST',
						dataType: 'json',
						success: function(dataSet,status,xhr) {	
							...
							table = $('#recorddisplay').DataTable( {
								...
								columnDefs: [
									...
									{ className: "dt-center", "targets": [ 0,1,2,3,6,7,8,9 ] },
			...
			function LoadSymbols()
			{
				try
				{
					...
					$.ajax({
						url: "<?php echo site_url('ui/Ordersrep_in/GetSymbols');?>",
						type: 'POST',
						dataType: 'json',
						success: function(data,status,xhr) {	
							$.unblockUI();
                    	...
                    	<table class="hover table table-bordered data-table display wrap" id="recorddisplay">
                          <thead>
                            <tr>
                                ...
                                <th style="text-align:center" width="10%">ORDER&nbsp;TYPE</th>
                                <th style="text-align:center" width="9%">BROKER&nbsp;ID</th>
