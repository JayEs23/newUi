...
<head>
	<title>Naija Art Market | Secondary Trades Report</title>
	<meta charset="utf-8">
		...
		$(document).ready(function(e) {
			...
			$('#btnPDF').click(function(e) 
			{
				try
				{
					...
					$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Secondary Trades PDF Report. Please Wait...</p>',theme: false,baseZ: 2000});
					...
					if (sdt==edt)
					{
						reportfile="secondarytradereport_"+sd+".pdf";
					}else
					{
						reportfile="secondarytradereport_from_"+sd+'_to_'+ed+".pdf";
					}
					...
					var mydata={email:em,symbol:sym,startdate:sdt,enddate:edt,ReportTitle:msg};	
																
					$.ajax({
						url: "<?php echo site_url('ui/Secondarytradesrep_in/CreatePDFReport'); ?>",
						data: mydata,
						type: 'POST',
						dataType: 'text',
			...
			function DisplayReport(em,sym,sdt,edt)
			{
				try
				{
					$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Retrieving Secondary Trades Records. Please Wait...</p>',theme: false,baseZ: 2000});
					...
					var mydata={email:em,symbol:sym,startdate:sdt,enddate:edt};	
					
					$('#recorddisplay > tbody').html('');
					
					$.ajax({
						url: "<?php echo site_url('ui/Secondarytradesrep_in/DisplayReport');?>",
						data: mydata,
						type: 'POST',
						dataType: 'json',
			...
			function LoadSymbols()
			{
				try
				{
					...
					$.ajax({
						url: "<?php echo site_url('ui/Secondarytradesrep_in/GetSymbols');?>",
						type: 'POST',
						dataType: 'json',
						success: function(data,status,xhr) {	
							$.unblockUI();
