		...
		$(document).ready(function(e) {
			...
			$('#btnPDF').click(function(e) 
			{
				try
				{
					...
					if (sdt==edt)
					{
						reportfile="bsrequestsinreport_"+sd+".pdf";
					}else
					{
						reportfile="bsrequestsinreport_from_"+sd+'_to_'+ed+".pdf";
					}
					...
					var mydata={investor_id:Uid,email:em,symbol:sym,market_type:mtyp,trans_type:ttyp,request_status:sta,startdate:sdt,enddate:edt,ReportTitle:msg};
																
					$.ajax({
						url: "<?php echo site_url('ui/Requestsrep_in/CreatePDFReport'); ?>",
						data: mydata,
						type: 'POST',
						dataType: 'text',
			...
			$('#btnDisplay').click(function(e)
			{
				try
				{
					...
					DisplayReport(em,sym,mtyp,ttyp,sta,sdt,edt);
				}catch(e)
				{
					$.unblockUI();
			...
			function DisplayReport(em,sym,mtyp,ttyp,sta,sdt,edt)
			{
				try
				{
					...
					var mydata={investor_id:Uid,email:em,symbol:sym,market_type:mtyp,trans_type:ttyp,request_status:sta,startdate:sdt,enddate:edt};	
					
					$('#recorddisplay > tbody').html('');
					
					$.ajax({
						url: "<?php echo site_url('ui/Requestsrep_in/DisplayReport');?>",
						data: mydata,
						type: 'POST',
						dataType: 'json',
			...
			function LoadSymbols()
			{
				try
				{
					...
					$.ajax({
						url: "<?php echo site_url('ui/Requestsrep_in/GetSymbols');?>",
						type: 'POST',
						dataType: 'json',
						success: function(data,status,xhr) {	
							$.unblockUI();
