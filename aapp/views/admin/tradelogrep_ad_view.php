...
<head>
    ...
    <title>Naija Art Mart - Trade Log Report</title>
	...
	$(document).ready(function(e) {
		...
		$('#btnPDF').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Trade Log PDF Report. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				if (sdt==edt)
				{
					reportfile="logtradesrep_ad_report_"+sd+".pdf";
				}else
				{
					reportfile="logtradesrep_ad_report_from_"+sd+'_to_'+ed+".pdf";
				}
				
				var msg;
									
				msg='Trade Log Report';					
				...
				$.ajax({
					url: "<?php echo site_url('admin/Tradelogrep_ad/CreatePDFReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
		...
		$('#btnExcel').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Trade Log Excel Report. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				if (sdt==edt)
				{
					reportfile="logtradesrep_ad_report_"+sd+".xls";
				}else
				{
					reportfile="logtradesrep_ad_report_from_"+sd+'_to_'+ed+".xls";
				}
				...
				$.ajax({
					url: "<?php echo site_url('admin/Tradelogrep_ad/CreateExcelReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
					complete: function(xhr, textStatus) {					
		...
		function DisplayReport(usr,sdt,edt)
		{
			try
			{
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Retrieving Trade Log Report Records. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				$.ajax({
					url: "<?php echo site_url('admin/Tradelogrep_ad/DisplayReport');?>",
					data: mydata,
					type: 'POST',
					dataType: 'json',
					success: function(dataSet,status,xhr) {	
						...
						table = $('#recorddisplay').DataTable( {
							...
							ordering: false,
							autoWidth:false,
							language: {zeroRecords: "No Trade Log Record Found"},
	...
	function LoadUsers()
	{
		try
		{
			...
			$.ajax({
				url: "<?php echo site_url('admin/Tradelogrep_ad/LoadUsers');?>",
				type: 'POST',
				dataType: 'json',
				success: function(data,status,xhr) {	
					$.unblockUI();
					...
					if ($(data).length > 0)
					{
						...
						{
							if (e.fullname) $("#cboUser").append(new Option($.trim(e.fullname), $.trim(e.fullname)));
						});//End each
					}	
				},
				error:  function(xhr,status,error) {
                    ...
                    <ul class="body-tabs-layout tabs-animated body-tabs-animated nav">
                        ...
                        <li class="nav-item">
                            <a role="tab" class="nav-link" id="tabView" data-toggle="tab" href="#view">
                                <span>View Trade Log Report</span>
                            </a>
                        </li>
                                           ...
                                           <table class="hover table table-bordered data-table display wrap " id="recorddisplay">
                                              <thead style="color:#ffffff; background-color:#7E7B7B;">
                                                <tr>
                                                    ...
                                                    <th style="text-align:center" width="25%">OPERATION</th>                                       				<th style="text-align:center;" width="49%">ACTIVITY</th>
                                                    <th style="text-align:center;" width="10%">USER&nbsp;IP</th>
                                                </tr>
                                              </thead>
                                            </table>                                           
                                        </div>
