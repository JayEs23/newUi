...
<head>
    ...
    <title>Naija Art Mart - NSE Commission Report</title>
	...
	$(document).ready(function(e) {
		...
		$('#btnPDF').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating NSE Commission PDF Report. Please Wait...</p>',theme: false,baseZ: 2000});				
				...
				if (sdt==edt)
				{
					reportfile="stockexcchange_op_report_"+sd+".pdf";
				}else
				{
					reportfile="stockexcchange_op_report_from_"+sd+'_to_'+ed+".pdf";
				}
				
				var msg;
									
				msg='NSE Commission Report';					
				...
				var mydata={startdate:sdt,enddate:edt,ReportTitle:msg};
															
				$.ajax({
					url: "<?php echo site_url('admin/Nsefeerep_op/CreatePDFReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
		...
		$('#btnExcel').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating NSE Commission Excel Report. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				if (sdt==edt)
				{
					reportfile="stockexcchange_op_report_"+sd+".xls";
				}else
				{
					reportfile="stockexcchange_op_report_from_"+sd+'_to_'+ed+".xls";
				}
				...
				$.ajax({
					url: "<?php echo site_url('admin/Nsefeerep_op/CreateExcelReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
					complete: function(xhr, textStatus) {					
		...
		function DisplayReport(sdt,edt)
		{
			try
			{
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Retrieving NSE Commission Report Records. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				$.ajax({
					url: "<?php echo site_url('admin/Nsefeerep_op/DisplayReport');?>",
					data: mydata,
					type: 'POST',
					dataType: 'json',
					success: function(dataSet,status,xhr) {	
						...
						table = $('#recorddisplay').DataTable( {
							...
							ordering: false,
							autoWidth:false,
							language: {zeroRecords: "No NSE Commission Record Found"},
						...
						if (parseInt(fee) > 0) 
						{
							$('#tdNSEFee').html(number_format (fee, 2, '.', ','));
						}else
						{
							$('#tdNSEFee').html('');
						}
                    ...
                    <ul class="body-tabs-layout tabs-animated body-tabs-animated nav">
                        ...
                        <li class="nav-item">
                            <a role="tab" class="nav-link" id="tabView" data-toggle="tab" href="#view">
                                <span>View NSE Commission Report</span>
                            </a>
                        </li>
                                           ...
                                           <table class="hover table table-bordered data-table display wrap " id="recorddisplay">
                                              <thead style="color:#ffffff; background-color:#7E7B7B;">
                                                <tr>
                                                    ...
                                                    <th style="text-align:right; padding-right:7px;" width="17%">NSE&nbsp;COMMISSION(₦)</th>
                                                </tr>
                                              </thead>

                                              <tfoot style="color:#ffffff; background-color:#7E7B7B;">
                                                    <tr>
                                                        <th colspan="6" style="text-align:right; padding:3px; padding-right:10px; font-weight:bold; font-size:13px;" width="83%">TOTAL NSE COMMISSION (₦):</th>
                                                        
                                                        <th id="tdNSEFee" style="text-align:right; padding:3px; padding-right:8px; font-weight:bold; font-size:13px;" width="17%"></th>
                                                    </tr>
                                              </tfoot>
                                            </table>                                           
                                        </div>
