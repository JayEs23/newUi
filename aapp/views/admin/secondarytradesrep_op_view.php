...
<head>
    ...
    <title>Naija Art Mart - Secondary Trades Report</title>
	...
	$(document).ready(function(e) {
		...
		$('#btnPDF').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Secondary Trade PDF Report. Please Wait...</p>',theme: false,baseZ: 2000});
				
				
				var buy=$.trim($('#cboBuyer').val());
				var buybr=$.trim($('#cboBuyerBroker').val());
				var sell=$.trim($('#cboSeller').val());
				var sellbr=$.trim($('#cboSellerBroker').val());
				...
				if (sdt==edt)
				{
					reportfile="secondarytrade_op_report_"+sd+".pdf";
				}else
				{
					reportfile="secondarytrade_op_report_from_"+sd+'_to_'+ed+".pdf";
				}
				
				var msg;
									
				msg='Secondary Trade Report';					
				...
				var mydata={buyer:buy,buyerbroker:buybr,seller:sell,sellerbroker:sellbr,symbol:sym,startdate:sdt,enddate:edt,ReportTitle:msg};
															
				$.ajax({
					url: "<?php echo site_url('admin/Secondarytradesrep_op/CreatePDFReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
		...
		$('#btnExcel').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Secondary Trade Excel Report. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				if (sdt==edt)
				{
					reportfile="secondarytrade_op_report_"+sd+".xls";
				}else
				{
					reportfile="secondarytrade_op_report_from_"+sd+'_to_'+ed+".xls";
				}
				...
				$.ajax({
					url: "<?php echo site_url('admin/Secondarytradesrep_op/CreateExcelReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
					complete: function(xhr, textStatus) {					
		...
		$('#btnDisplay').click(function(e)
		{
			try
			{
				...
				DisplayReport(buy,buybr,sell,sellbr,sym,sdt,edt);
			}catch(e)
			{
				$.unblockUI();
		...
		function DisplayReport(buy,buybr,sell,sellbr,sym,sdt,edt)
		{
			try
			{
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Retrieving Secondary Trades Report Records. Please Wait...</p>',theme: false,baseZ: 2000});
					
				//Make Ajax Request
				var msg;
																		
				msg='Secondary Trades Report';					
				...
				var mydata={buyer:buy,buyerbroker:buybr,seller:sell,sellerbroker:sellbr,symbol:sym,startdate:sdt,enddate:edt};	
				
				$('#recorddisplay > tbody').html('');
				
				$.ajax({
					url: "<?php echo site_url('admin/Secondarytradesrep_op/DisplayReport');?>",
					data: mydata,
					type: 'POST',
					dataType: 'json',
					success: function(dataSet,status,xhr) {	
						...
						table = $('#recorddisplay').DataTable( {
							...
							ordering: false,
							autoWidth:false,
							language: {zeroRecords: "No Secondary Trades Record Found"},
	...
	function LoadInvestors()
	{
		try
		{
			...
			$.ajax({
				url: "<?php echo site_url('admin/Secondarytradesrep_op/GetInvestors');?>",
				type: 'POST',
				dataType: 'json',
				success: function(data,status,xhr) {	
					$.unblockUI();
					...
					if ($(data).length > 0)
					{
						...
						$.each($(data), function(i,e)
						{
							if (e.user_name && e.email)
							{
								...
								$("#cboSeller").append(new Option($.trim(e.user_name)+" ("+$.trim(e.email)+")", $.trim(e.email)));
							}
						});//End each
					}	
				},
	...
	function LoadBrokers()
	{
		try
		{
			$.blockUI({message: '<img src="<?php echo base_url();?>images/loader.gif" /><p>Loading Seller And Buyer Brokers. Please Wait...</p>',theme: false,baseZ: 2000});				

			$('#cboBuyerBroker').empty();		
			$('#cboSellerBroker').empty();		

			$.ajax({
				url: "<?php echo site_url('admin/Secondarytradesrep_op/GetBrokers');?>",
				type: 'POST',
				dataType: 'json',
				success: function(data,status,xhr) {	
					$.unblockUI();
					...
					if ($(data).length > 0)
					{
						...
						$("#cboSellerBroker").append(new Option("[ALL SELLER BROKERS]", ""));
						...
						$.each($(data), function(i,e)
						{
							if (e.company && e.broker_id)
							{
								...
								$("#cboSellerBroker").append(new Option($.trim(e.company), $.trim(e.broker_id)));
							}
						});//End each
					}	
				},
	...
	function LoadSymbols()
	{
		try
		{
			...
			$.ajax({
				url: "<?php echo site_url('admin/Secondarytradesrep_op/GetSymbols');?>",
				type: 'POST',
				dataType: 'json',
				success: function(data,status,xhr) {	
					$.unblockUI();
                    ...
                    <ul class="body-tabs-layout tabs-animated body-tabs-animated nav">
                        ...
                        <li class="nav-item">
                            <a role="tab" class="nav-link" id="tabView" data-toggle="tab" href="#view">
                                <span>View Secondary Trades Report</span>
                            </a>
                        </li>
                            ...
                            <div class="main-card mb-3 card">
                                <div class="card-body">
                                    <form class="">
                                        ...
                                        <!--Seller-->
                                        <div title="Seller" class="position-relative row form-group">
                                        	<label for="cboSeller" class="col-sm-2 col-form-label">Seller</label>
                                        ...
                                        <!--Seller Broker-->
                                        <div title="Seller Broker" class="position-relative row form-group">
                                        	<label for="cboSellerBroker" class="col-sm-2 col-form-label">Seller Broker</label>
                                            
                                            <div class="col-sm-10">
                                            	<select class="form-control" id="cboSellerBroker"></select>
                                             </div>
                                        </div>
                                           ...
                                           <table class="hover table table-bordered data-table display wrap " id="recorddisplay">
                                              <thead style="color:#ffffff; background-color:#7E7B7B;">
                                                <tr>
                                                    ...
                                                    <th title="Total transaction amount with commissions and fees" style="text-align:right; padding-right:7px;" width="12%">TOTAL&nbsp;AMOUNT(₦)</th>                                
                                              ...
                                              <tfoot style="color:#ffffff; background-color:#7E7B7B;">
                                                    <tr>
                                                        <th colspan="5" style="text-align:right; padding:3px; padding-right:10px; font-weight:bold; font-size:13px;" width="54%">TOTAL AMOUNT (₦):</th>
