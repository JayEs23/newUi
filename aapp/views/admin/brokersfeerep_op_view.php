...
<head>
    ...
    <title>Naija Art Mart - Brokers Commission Report</title>
	...
	$(document).ready(function(e) {
		...
		$('#btnPDF').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Brokers Commission PDF Report. Please Wait...</p>',theme: false,baseZ: 2000});
				
				
				var cd=$.trim($('#cboBroker').val());			
				...
				if (sdt==edt)
				{
					reportfile="brokerfee_op_report_"+sd+".pdf";
				}else
				{
					reportfile="brokerfee_op_report_from_"+sd+'_to_'+ed+".pdf";
				}
				
				var msg;
									
				msg='Brokers Commission Report';					
				...
				var mydata={recipient_code:cd,startdate:sdt,enddate:edt,ReportTitle:msg};
															
				$.ajax({
					url: "<?php echo site_url('admin/Brokersfeerep_op/CreatePDFReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
		...
		$('#btnExcel').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Brokers Commission Excel Report. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				if (sdt==edt)
				{
					reportfile="brokerfee_op_report_"+sd+".xls";
				}else
				{
					reportfile="brokerfee_op_report_from_"+sd+'_to_'+ed+".xls";
				}
				...
				$.ajax({
					url: "<?php echo site_url('admin/Brokersfeerep_op/CreateExcelReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
					complete: function(xhr, textStatus) {					
		...
		function DisplayReport(cd,sdt,edt)
		{
			try
			{
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Retrieving Brokers Commission Report Records. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				var mydata={recipient_code:cd,startdate:sdt,enddate:edt};	
				
				$('#recorddisplay > tbody').html('');
				
				$.ajax({
					url: "<?php echo site_url('admin/Brokersfeerep_op/DisplayReport');?>",
					data: mydata,
					type: 'POST',
					dataType: 'json',
					success: function(dataSet,status,xhr) {	
						...
						table = $('#recorddisplay').DataTable( {
							...
							ordering: false,
							autoWidth:false,
							language: {zeroRecords: "No Brokers Commission Record Found"},
							...
							columns: [
								...
								{ width: "13%" },//Tokens
								{ width: "13%" },//Price
								{ width: "17%" },//Trade Amount
								{ width: "17%" }//Broker Commission
						...
						fee=table.column(6).data().sum();
						
						if (parseInt(fee) > 0) 
						{
							$('#tbBrokerFee').html(number_format (fee, 2, '.', ','));
						}else
						{
							$('#tbBrokerFee').html('');
						}
	...
	function LoadBrokers()
	{
		try
		{
			...
			$.ajax({
				url: "<?php echo site_url('admin/Brokersfeerep_op/GetBrokers');?>",
				type: 'POST',
				dataType: 'json',
				success: function(data,status,xhr) {	
					$.unblockUI();
					...
					if ($(data).length > 0)
					{
						...
						{
							if (e.company && e.recipient_code)
							{
								$("#cboBroker").append(new Option($.trim(e.company), $.trim(e.recipient_code)));
							}
						});//End each
					}	
				},
                    ...
                    <ul class="body-tabs-layout tabs-animated body-tabs-animated nav">
                        ...
                        <li class="nav-item">
                            <a role="tab" class="nav-link" id="tabView" data-toggle="tab" href="#view">
                                <span>View Brokers Commission Report</span>
                            </a>
                        </li>
                            ...
                            <div class="main-card mb-3 card">
                                <div class="card-body">
                                    <form class="">
                                        ...
                                        <!--Brokers-->
                                        <div title="Brokers" class="position-relative row form-group">
                                        	<label for="cboBroker" class="col-sm-2 col-form-label">Brokers</label>
                                           ...
                                           <table class="hover table table-bordered data-table display wrap " id="recorddisplay">
                                              <thead style="color:#ffffff; background-color:#7E7B7B;">
                                                <tr>
                                                    ...
                                                    <th style="text-align:center" width="17%">TRADE&nbsp;ID</th>
                                                    <th style="text-align:center" width="13%">ASSET</th>
                                                    <th style="text-align:center" width="13%">TOKENS</th>
                                                    <th style="text-align:right; padding-right:7px;" width="13%">PRICE(₦)</th>
                                                    <th style="text-align:right; padding-right:7px;" width="17%">TRADE&nbsp;AMOUNT(₦)</th>
                                                    <th style="text-align:right; padding-right:7px;" width="17%">BROKER&nbsp;COMMISSION(₦)</th>
                                                </tr>
                                              </thead>

                                              <tfoot style="color:#ffffff; background-color:#7E7B7B;">
                                                    <tr>
                                                        <th colspan="6" style="text-align:right; padding:3px; padding-right:10px; font-weight:bold; font-size:13px;" width="83%">TOTAL BROKER COMMISSION (₦):</th>
                                                        
                                                        <th id="tbBrokerFee" style="text-align:right; padding:3px; padding-right:8px; font-weight:bold; font-size:13px;" width="17%"></th>
                                                    </tr>
                                              </tfoot>
                                            </table>                                           
                                        </div>
