...
<head>
    ...
    <title>Naija Art Mart - Issuers Payments Report</title>
	...
	$(document).ready(function(e) {
		...
		$('#btnPDF').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Issuers Payment PDF Report. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				if (sdt==edt)
				{
					reportfile="issuerpayment_op_report_"+sd+".pdf";
				}else
				{
					reportfile="issuerpayment_op_report_from_"+sd+'_to_'+ed+".pdf";
				}
				
				var msg;
									
				msg='Issuers Payment Report';					
				...
				var mydata={payment_status:sta,issuer_email:sell,symbol:sym,startdate:sdt,enddate:edt,ReportTitle:msg};
															
				$.ajax({
					url: "<?php echo site_url('admin/Issuerspaymentrep_op/CreatePDFReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
		...
		$('#btnExcel').click(function(e) 
		{
			try
			{
				...
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Generating Issuers Payment Excel Report. Please Wait...</p>',theme: false,baseZ: 2000});
				...
				if (sdt==edt)
				{
					reportfile="issuerpayment_op_report_"+sd+".xls";
				}else
				{
					reportfile="issuerpayment_op_report_from_"+sd+'_to_'+ed+".xls";
				}
				...
				$.ajax({
					url: "<?php echo site_url('admin/Issuerspaymentrep_op/CreateExcelReport'); ?>",
					data: mydata,
					type: 'POST',
					dataType: 'text',
					complete: function(xhr, textStatus) {					
		...
		$('#btnDisplay').click(function(e)
		{
			try
			{
				...
				DisplayReport(sta,sell,sym,sdt,edt);
			}catch(e)
			{
				$.unblockUI();
		...
		function DisplayReport(sta,sell,sym,sdt,edt)
		{
			try
			{
				$.blockUI({message:'<img src="<?php echo base_url();?>images/loader.gif" /><p>Retrieving Issuers Payments Report Records. Please Wait...</p>',theme: false,baseZ: 2000});
					
				//Make Ajax Request
				var msg;
																		
				msg='Issuers Payments Report';					
				...
				var mydata={payment_status:sta,issuer_email:sell,symbol:sym,startdate:sdt,enddate:edt};	
				
				$('#recorddisplay > tbody').html('');
				
				$.ajax({
					url: "<?php echo site_url('admin/Issuerspaymentrep_op/DisplayReport');?>",
					data: mydata,
					type: 'POST',
					dataType: 'json',
					success: function(dataSet,status,xhr) {	
						...
						table = $('#recorddisplay').DataTable( {
							...
							ordering: false,
							autoWidth:false,
							language: {zeroRecords: "No Issuers Payments Record Found"},
							...
							columnDefs: [
								...
								{ className: "dt-center", "targets": [ 0,1,2,3,6,7,8 ] },
							...
							columns: [
								...
								{ width: "10%" },//Price
								{ width: "13%" },//Issuer Fee
								{ width: "11%" },//Transfer Code
								{ width: "16%" },//Seller Email
								{ width: "9%" }//Payment Status
	...
	function LoadIssuers()
	{
		try
		{
			...
			$.ajax({
				url: "<?php echo site_url('admin/Issuerspaymentrep_op/GetIssuers');?>",
				type: 'POST',
				dataType: 'json',
				success: function(data,status,xhr) {	
					$.unblockUI();
	...
	function LoadSymbols()
	{
		try
		{
			...
			$.ajax({
				url: "<?php echo site_url('admin/Issuerspaymentrep_op/GetSymbols');?>",
				type: 'POST',
				dataType: 'json',
				success: function(data,status,xhr) {	
					$.unblockUI();
                    ...
                    <ul class="body-tabs-layout tabs-animated body-tabs-animated nav">
                        ...
                        <li class="nav-item">
                            <a role="tab" class="nav-link" id="tabView" data-toggle="tab" href="#view">
                                <span>View Issuers Payments Report</span>
                            </a>
                        </li>
                                ...
                                <div class="card-body">
                                    <form class="">
                                        ...
                                        <div title="Issuer" class="position-relative row form-group">
                                        	<label for="cboIssuer" class="col-sm-2 col-form-label">Issuer</label>
                                        ...
                                        <!--Payment Status-->
                                        <div title="Payment Status" class="position-relative row form-group">
                                        	<label for="cboStatus" class="col-sm-2 col-form-label">Asset</label>
                                        ...
                                        <div class="card-body">
                                           <table class="hover table table-bordered data-table display wrap " id="recorddisplay">
                                              <thead style="color:#ffffff; background-color:#7E7B7B;">
                                                <tr>
                                                    <th style="text-align:center" width="9%">TRADE&nbsp;DATE</th>
                                                    <th style="text-align:center" width="12%">TRADE&nbsp;ID</th>
                                                    ...
                                                    <th style="text-align:right; padding-right:7px;" width="13%">ISSUER&nbsp;FEE(₦)</th>
                                                    <th style="text-align:center" width="11%">TRANSFER&nbsp;CODE</th>
                                                    <th style="text-align:center" width="16%">ISSUER&nbsp;EMAIL</th>
                                                    <th style="text-align:center" width="9%">PAYMENT&nbsp;STATUS</th>
                                                </tr>
                                              </thead>
                                              ...
                                              <tfoot style="color:#ffffff; background-color:#7E7B7B;">
                                                    <tr>
                                                        <th colspan="5" style="text-align:right; padding:3px; padding-right:10px; font-weight:bold; font-size:13px;" width="51%">TOTAL ISSUER/SELLER FEE (₦):</th>
                                                        
                                                        <th id="tdIssuerFee" style="text-align:right; padding:3px; padding-right:8px; font-weight:bold; font-size:13px;" width="13%"></th>
                                                        
                                                        <th style="text-align:right; padding:3px; padding-right:8px; font-weight:bold; font-size:12px;" width="11%"></th>
                                                        
                                                        <th style="text-align:right; padding:3px; padding-right:8px; font-weight:bold; font-size:12px;" width="16%"></th>
                                                        <th style="text-align:right; padding:3px; padding-right:8px; font-weight:bold; font-size:12px;" width="9%"></th>
                                                    </tr>
                                              </tfoot>
                                            </table>                                           
                                        </div>
